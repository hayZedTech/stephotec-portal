...
export default function AssignmentManager() {
    ...
    const handleAssignToGroup = async () => {
        ...
        try {
            ...
            successToast("Assignment assigned to group members!");
            const group = groups.find(g => g.id === selectedGroupId);
            if (group && group.members_detail) {
                const newAssigned = new Set(assignedStudents);
                group.members_detail.forEach(m => newAssigned.add(m.id));
    ...
    const handleUnassignFromGroup = async () => {
        ...
        try {
            ...
            successToast("Assignment removed from group members!");
            const group = groups.find(g => g.id === selectedGroupId);
            if (group && group.members_detail) {
                const newAssigned = new Set(assignedStudents);
                group.members_detail.forEach(m => newAssigned.delete(m.id));
                setAssignedStudents(newAssigned);
            }
            setSelectedGroupId("");
            loadStudents(true);
        ...
        confirmAction(
            ...
            async () => {
                try {
                    ...
                    const newAssigned = new Set(assignedStudents);
                    selectedAssignedStudents.forEach(id => newAssigned.delete(id));
                    setAssignedStudents(newAssigned);
                    setSelectedAssignedStudents(new Set());
                    loadStudents(true);
                } catch (error) {
